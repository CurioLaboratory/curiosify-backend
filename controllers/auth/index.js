...
const loadSecrets = async () => {
  try {
    const data = await secretsManager
      .getSecretValue({ SecretId: "curiosify-backend-secrets" })
      .promise();
      ...
      if (!passwordMatch) {
        ...
        res.status(200).json({
          ...
          user: {
            ...
            rollNo: user.role === "student" ? user.rollNo : undefined,
            classLevel: user.role === "student" ? user.classLevel : undefined,
          },
        });
      }
    }
...
exports.verifyEmail = async (req, res) => {
  try {
    ...
    if (!userData) {
      return res
        .status(404)
        .json({ success: false, message: "User data not found or expired." });
    }
